...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    //arxikopoioume  auta pou prosthesame sto db.h
    pthread_cond_init(&(self->rexists),NULL);
	pthread_cond_init(&(self->wexists),NULL);

    pthread_mutex_init(&(self->mutexx),NULL);
...
{ 
    //kleidwnoume mutex
    pthread_mutex_lock(&self->mutexx);
	while (self->rcounter >= 1)
	      pthread_cond_wait(&self->rexists,&self->mutexx);
	self->flag = true;
    ...
    //pairname to memtable_add se metavliti val, etsi wste na gieni unlock kai meta na kanei return to val i sunartisi

    int val =memtable_add(self->memtable, key, value);
	self->flag = false;
	pthread_cond_signal(&self->wexists);
    //ksekleidwnoume mutex 
    pthread_mutex_unlock(&self->mutexx);
    return val;
}
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    pthread_mutex_lock(&self->mutexx);
    //an to flag einai true, to trexon nima mpainei se anamoni me pthread_cond_wait me paremetro wexists
    while (self->flag == true)
	      pthread_cond_wait(&self->wexists,&self->mutexx);
    //afou den uparxei perisptwsi na mpei se while loop auksanoume to rcounter++
	self->rcounter++;
    pthread_mutex_unlock(&self->mutexx);
    //prospathei sto memtable na vrei to kleidi gia tin timh
    ...
    {   
         ...
         val=sst_get(self->sst, key, value);
    }
    ...
    //stelnei sima 
    if ( self->rcounter == 0)
	     pthread_cond_signal(&self->rexists);
    pthread_mutex_unlock(&self->mutexx);
